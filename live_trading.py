...
class alpaca_trader:
    ...
    def log(self, start=False, message=None):
        ...
        if start:
            try:
                fh = open(f"LiveTradingLogs/{datetime.today().strftime('%Y-%m-%d')}.txt", 'x')
                fh.close()
        ...
        if message:
            try:
                fh = open(f"LiveTradingLogs/{datetime.today().strftime('%Y-%m-%d')}.txt", 'a')
    ...
    def print_account_details(self):
        ...
        #print parameters to console to check weights
        debug_params = False
        if (debug_params):
            print("\nCurrent parameters:")
    ...
    def trade(self):
        try:
            ...
            self.subscribe_positions()  # Try to get any open positions
    ...
    async def update(self):
       # Periodically update stocks
        ...
        try:
            #check market hours
            self.check_time()
            if not self.market_open: #if market is closed, close connection and exit
                self.ws.close()
                alpacawebsocket.alpaca = None
                main.wait_until_open(seconds = (self.trading_client.get_clock().next_open - datetime.now(timezone.utc)).total_seconds())
                return
                
            
            #if new hour, update all stocks
            #print(f"minute: {self.get_minute()}") #debug
            print("New hour: calling updates.\n")
            
            if self.get_minute() == 0:
                for stock in self.stocks.values():
                    try:
                        self.aggregate_bars(stock)
                        
                        #tick stock timer --- REVISIT THIS - IF STOCKS ARE LOADED FROM FILE, TIMER WILL BE WRONG. NEED SOMETHING LIKE EXIT BARS
                        stock.timer -= 1
                        if stock.timer < 0: stock.timer = 0
                    except Exception as e:
                        print(f"Error updating stock {stock.ticker}: {e}")
                        
                #place trades now
                print("Stocks updated. Checking to place trades.\n")
                self.place_buys()
                self.place_sells() # -----we want to place sells more often than at the hour most likely at some point
         
            # Subscribe to live data for tickers
            if self.stock_check_hours >= 4 and self.market_open: #update ticker list every 4 hours --should we do this every hour? just hold the tickers for their timer defined in stock data?
                self.stock_check_hours = 0
        ...
        #show stocks tracking and account info
        ...
        for position in positions:
            try:
                ...
                print(f"{ticker} -- {qty} shares {side} @ {avg_entry} with current pps of {current_price}, Market value: {market_value} for unrealized pl of {unrealized_pl}")
            except Exception as e:
                print (f"Error checking position: {e}")
        if positions == []:
            print("None")
         
        #sleep until next hour and run again
        time_until_next_hour = 60*(60-self.get_minute())
        #print(f"Time until next hour: {time_until_next_hour}")
        
        await asyncio.sleep(time_until_next_hour)  # Update every hour --this is in seconds
        asyncio.create_task(self.update())
    ...
    def subscribe_positions(self):
        try:
            ...
            for position in positions:
                ...
                #self.stocks[ticker].position_size = float(position.quantity) #are these necessary?
            ...
            if not tickers == []:
                try:
                    alpacawebsocket.subscribe_bars(self.ws, tickers)
                    print(f"Subscribed to open positions: {tickers}")
                except Exception as e:
    ...
    def get_new_stocks(self, tickerlist=[]):
        #get trending tickers
        tickers = st.get_trending_stocks()
        print(f"Checking subscription eligibility for {tickers}")
        
        
        
        #testing to see if we can update to new stocks -- DELETE WHEN TESTING IS DONE
        if self.first:
        ...
        #remove non-equity
        tickers = [ticker for ticker in tickers if yf.check_equity(ticker)]
            ...
            try:
                ...
                if not position and self.stocks[ticker].timer <= 0 and not ticker in tickers:
                    try:
                        ...
                        unsubs.append(ticker)
                    except Exception as e:
                        print(f"Unable check positions or pop stocks: {e}")
        ...
        if not unsubs == []:
            try:
                alpacawebsocket.unsubscribe_bars(self.ws,unsubs)
                print(f"Unsubscribed from {unsubs}")
            except Exception as e:
                print(f"Error unsubscribing from {unsubs}: {e}")
            ...
            try:
                if ticker not in self.stocks:
                    try:
                        ...
                        print(f"Error creating stock data for {ticker}: {e}")
                        tickers.remove(ticker)
            except Exception as e:
                print(f"Error creating stocks or checking tickers: {e}")
        try:
            alpacawebsocket.subscribe_bars(self.ws, tickers)
            print(f"Subscribed to {tickers}")
        except Exception as e:
            print(f"Error subscribing to tickers: {e}")
        time.sleep(3) # wait for these to go through before doing anything else
     
        
    #aggregate minute bars
    
    def aggregate_bars(self, stock):
        if stock.minute_bar_count != 0:
            # Convert timestamp column to datetime format and set as index
            print(stock.minute_bars.head()) #DEBUG
            
            #stock.minute_bars["timestamp"] = pd.to_datetime(stock.minute_bars["timestamp"])
            stock.minute_bars.set_index("timestamp", inplace=True)
            
            #fill emptys with nan to properly index
            
            start = stock.minute_bars.index.min().floor('min')
            end = stock.minute_bars.index.max().ceil('min')
            full_index = pd.date_range(start=start, end=end, freq='1min')
            stock.minute_bars = stock.minute_bars.reindex(full_index)
    ...
    def update_stock_from_callback(self, bar, stock):
        ...
        new_row = new_row.set_index('timestamp', inplace=True)
        stock.add_minute_bar(new_row)
        
        #print(f"Minute bar count: {bar['symbol']} - {stock.minute_bar_count}\n")
        #print(f"Minute: {self.get_minute()}")
    ...
    def bar_callback(self, bar):        
        if self.market_open: #ensure market is open to place trades and analyze bars
            #print(f"Bar received for {bar['symbol']} at {datetime.now(timezone.utc)}")
            ...
            try:
                ...
                self.update_stock_from_callback(bar, stock)
                ...
                print(f"Error processing bar for {bar['symbol']}: {e}")
    ...
    def place_buys(self):
        ...
        buy_weights = {}
        log_message = f"{datetime.now(timezone.utc)}: "
        for stock in self.stocks.values():
            buy_weights[stock.ticker] = stock.buy_weight
            self.log(message=log_message+f" {stock.ticker} - Weight: {stock.buy_weight}")
        ...
        print(f"Buy_Weight: {buy_weight}, Stock: {max_stock.ticker}")
        
        if not self.check_position(ticker=max_stock.ticker):
            if buy_weight != 0:
                stake = self.set_stake(max_stock, buy_weight/100)
                if stake > 0:
            ...
            try:
                ...
                for date in formatted_dates:
                    if date in stock.earnings:
                        stock.buy_weight = 0
                        print(f"Earnings date too recent or upcoming for {stock.ticker}, buy weight set to 0.")
            except Exception as e:
                print(f"Error checking earnings date for ticker {stock.ticker}: {e}")
                
        print(f"Buy weight for {stock.ticker}: {stock.buy_weight}")
    
    def save(self, stock): #call this when an order has been placed so we don't lose data on when it was placed
        try:
            with open(f"LiveStockData/{self.ticker}", "wb") as file:
                pickle.dump(stock, file)
        except Exception as e:
            print(f"Failed to save stock data for {stock.ticker}: {e}")
            
    def load(self, ticker): #try to load data about order execution per stock
        try:
            with open(f"LiveStockData/{ticker}", "rb") as file:
                self.stocks[ticker] = pickle.load(file)
                
                #make sure data is up to date
                self.stocks[ticker].get_pandas_data()
                
                #udpate bars_since_execution
                if self.stocks[ticker].position_size is not None:
                    self.stocks[ticker].bars_since_execution = (datetime.now(timezone.utc) - self.stocks[ticker].buy_time).total_seconds() / 3600 #turns into hourly number 
                
        except Exception as e:
            print(f"Failed to load stock data for {ticker}: {e}")
...
class stock_data:
    def __init__(self, ticker, period, historical_client, pandasdata=None):
        ...
        if pandasdata == None:
            try:
                ...
                print(f"Error initializing pandas data for {self.ticker}: {e}")
                return
            if self.pandasdata.empty:
                print(f"Error initializing pandas data for {self.ticker}: empty")
                return
        ...
        try:
            ...
            print(f"Error initializing indicators for {self.ticker}: {e}")
            return
        ...
        try:
            ...
            print(f"Error initalizing stock data for {self.ticker}: {e}")
            return
    ...
    def update_data(self, new_row):
        #call this at hour, add datalines to update indicators
        ...
        try:
            ...
            try:
                ...
                print(f"Error calcuating indicators or setting stock data for {self.ticker}: {e}")
                return
    ...
    def get_pandas_data(self):
        ...
        try:
            ...
            self.pandasdata = self.historical_client.get_stock_bars(StockBarsRequest(
                ...
                start=datetime.now(timezone.utc)-timedelta(weeks=52), #we download 52 weeks here to properly initialize 52-wk high and low ... maybe want a better solution?
                end=datetime.now(),
                adjustment='raw',
                feed=self.feed
            )).df
    ...
    def calculate_indicators(self, pandasdata=None):
        ...
        recent_data = self.pandasdata.tail(45) #past 5 trading days
    ...
    def get_stock_data(self):
        ...
        if (self.week_52_high - self.week_52_low == 0):
            self.pricentile = 0
        else:
            self.pricentile = (self.share_price - self.week_52_low) / (self.week_52_high - self.week_52_low)
